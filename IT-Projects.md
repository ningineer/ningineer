# IT Solutions and Engineering Projects

Welcome to my portfolio of IT projects! This repository contains various projects focused on different aspects of IT infrastructure, security, cloud engineering, and automation.

## Table of Contents
1. [Cloud Solutions & Security](#cloud-solutions-security)
2. [Network & System Security](#network-system-security)
3. [Automation & Scripting](#automation-scripting)
4. [Infrastructure Management & Monitoring](#infrastructure-managment-monitoring)
5. [DevOps & Continuous Integration/Continuous Deployment](#devops-continuous-integration-continuous-deployment)
6. [Threat Detection & Mitigation](#threat-detection-mitigation)

---

## Cloud Solutions & Security
Projects that focus on cloud infrastructure management, cloud security, and cloud-based solutions.

- **Basic: Cloud Storage with Encryption**
  - Description:
  - [Link to Project Folder]

- **Intermediate: Cloud Security Automation with AWS Lambda**  
  - Description: Automated workflows and deployments using AWS Lambda.
  - [Link to Project Folder]

- **Advanced: Cloud Security Posture Management with AWS Security Hub**  
  - Description: Implemented security monitoring and compliance checks using AWS Security Hub.
  - [Link to Project Folder]

---

## Network & System Security
Projects that deal with network security, vulnerability assessments, and infrastructure audits.

- **Basic: JWipe (Disk Wiping Utility)**
  - Description: Developed a utility to securely wipe disks, ensuring all data is irrecoverable by overwriting sectors with random data multiple times, adhering to data security standards.
  - [Link to Project Folder]

- **Intermediate: Nmap Network Mapping & Audit**  
  - Description: Python-based script for network mapping and security audits.
  - [Network Mapping and Security Audit](https://github.com/ningineer/Network_Mapping_and_Audit)

- **Intermediate: Failed RDP Detection with GeoLocation and Azure Sentinel**  
  - Description: This project leverages Azure Sentinel to track failed RDP login attempts, enriches the data with geographic location information, and helps monitor and defend against potential brute force attacks on your system.
  - [AzureSentinel RDP GeoTracker](https://github.com/ningineer/AzureSentinel_RDP_GeoTracker)

- **Intermediate: Keylogger Script**  
  - Description: A Python script that logs keystrokes for auditing purposes.  
  - [KeyLogger Project](https://github.com/ningineer/Keylogger_Project)

- **Intermediate: FIM (File Integrity Monitor)**
  - Description: Developed a File Integrity Monitoring solution to detect unauthorized changes to critical files and directories, strengthening system security and monitoring for potential intrusions.
  - [Link to Project Folder]

- **Advanced:**
---

## Automation & Scripting
Projects focused on automating tasks and processes using Python and other scripting languages.

- **Automated Log Parsing and Analysis**  
  - Description: A script that automates log file analysis for security events.
  - [Link to Project Folder]

- **Log File Analysis with Python**  
  - Description: Python script for parsing and analyzing server log files to detect anomalies.  
  - [Log File Analysis with Python](https://github.com/ningineer/Log_File_Analysis_with_Python)

- **Active Directory Bulk User Creation**
  - Description: Automated the creation of multiple user accounts in Active Directory using PowerShell scripts, improving efficiency and reducing manual input errors in user management.
  - [Link to Project Folder]
 
- **Package Delivery Application (Data Structures and Algorithms Demo)**
  - Description: Built a package delivery application that demonstrates various data structures and algorithms, showcasing optimization techniques for route planning and package sorting.
  - [Link to Project Folder]

- **Real-Time Server Monitoring with Python**  
  - Description: Automated server monitoring script using Python and Prometheus.
  - [Link to Project Folder]

---

## Infrastructure Management & Monitoring
Projects that involve monitoring and maintaining infrastructure, including hardware and software systems.

- **Basic: Server Health Monitoring with Nagios**
  - Description: Implemented Nagios to monitor the health of servers and critical services, such as CPU load, memory usage, and disk space.
  - [Link to Project Folder]

- **Intermediate: Automated Server Provisioning with Ansible**
  - Description: Developed Ansible playbooks to automate the provisioning of servers, ensuring consistent configurations, and implementing security practices.
  - [Link to Project Folder]

- **Advanced: Comprehensive Infrastructure Monitoring with Prometheus and Grafana**

    Description: Set up Prometheus for metrics collection and Grafana for real-time infrastructure monitoring, creating detailed dashboards for system health and performance.
    [Link to Project Folder]
    
- **Machine Learning Model Deployment with Python**
  - Description: Deployed machine learning models in cloud environments using Python, integrating services like AWS Sagemaker and Azure to scale predictions and analyze data effectively.
  - [Link to Project Folder]

---

## DevOps & Continuous Integration/Continuous Deployment (CI/CD)
Initiatives that streamline development and operations processes, focusing on automation, continuous integration, and continuous delivery to optimize software deployment and system performance.

- **Basic: CI/CD Pipeline with GitHub Actions**
  - Description: Automated the testing and deployment of a Node.js application to a cloud environment using GitHub Actions CI/CD pipeline.
  - [Link to Project Folder]

- **Intermediate: Automated Testing with Jenkins**
  - Description: Integrated Jenkins with GitHub to run unit and integration tests automatically, setting up email notifications for test failures.
  - [Link to Project Folder]

- **Advanced: Infrastructure as Code with Terraform and AWS**
  - Description: Implemented Infrastructure as Code using Terraform to provision AWS resources and set up an automated deployment pipeline with Jenkins.
  - [Link to Project Folder]
    
- **Cloud Deployment with Python & AWS SDK**
  - Description: Utilized Python and the AWS SDK to automate cloud resource deployment, enabling scalable infrastructure management on AWS for various application needs.
  - [Link to Project Folder]

---

## Threat Detection & Mitigation
Projects focused on identifying, analyzing, and mitigating security threats through real-time monitoring, advanced detection techniques, and automated responses to enhance system resilience and data protection.

- **Basic: Failed Login Detection with Syslog (Linux)**
  - Description: Wrote a Python script to parse syslog logs for failed login attempts and trigger alerts when a threshold is exceeded.
  - [Link to Project Folder]

- **Intermediate: Ransomware Detection with File Integrity Monitoring (FIM)**
  - Description: Deployed FIM tools like OSSEC to monitor files for unauthorized modifications commonly made by ransomware and configured real-time alerts.
  - [Link to Project Folder]

- **Advanced: Threat Detection with Machine Learning**
  - Description: Built a machine learning model to detect network traffic anomalies, such as botnet activity, using supervised learning techniques.
  - [Link to Project Folder]

